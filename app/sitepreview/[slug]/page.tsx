...
export default async function ProspectSitePreviewPage({ params }: PageProps) {
  ...
  const heroSurface = isMechanicPreview ? 'bg-[#07080a]' : 'bg-[#132316]'
    ...
    <div className={`min-h-screen overflow-x-hidden ${isMechanicPreview ? 'bg-[#f2f0ec] text-[#161719]' : 'bg-[#f5f1e8] text-[#182417]'}`} style={{ fontFamily: 'Inter, system-ui, sans-serif', letterSpacing: 0 }}>
      ...
      <main>
        ...
        >
          {isMechanicPreview ? (
            <>
              <div className="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-[linear-gradient(90deg,rgba(229,9,20,0.18)_1px,transparent_1px),linear-gradient(0deg,rgba(255,255,255,0.08)_1px,transparent_1px)] bg-[size:56px_56px] opacity-70" />
              {logoImage ? (
                <img src={logoImage} alt="" aria-hidden="true" className="pointer-events-none absolute -right-28 top-20 hidden w-[34rem] rotate-[-9deg] opacity-[0.13] drop-shadow-[0_0_80px_rgba(229,9,20,0.35)] lg:block" />
              ) : null}
            </>
          ) : null}
          <div className="mx-auto grid w-full max-w-7xl gap-8 lg:grid-cols-[0.92fr_0.58fr] lg:items-end">
            ...
            <aside className={`rounded-[28px] border ${isMechanicPreview ? 'border-[#e50914]/42 bg-[#08090b]/92 text-white' : 'border-white/22 bg-[#f8f3e7]/94 text-[#172315]'} p-5 shadow-[0_26px_80px_rgba(0,0,0,0.28)] backdrop-blur-md`}>
              {logoImage ? (
                <div className={`mb-5 flex items-center gap-4 rounded-[24px] border p-4 ${isMechanicPreview ? 'border-[#e50914]/34 bg-[#e50914]/10' : 'border-[#d7ccb7] bg-white'}`}>
                  <span className={`grid h-28 w-28 shrink-0 place-items-center overflow-hidden ${isMechanicPreview ? 'drop-shadow-[0_0_24px_rgba(229,9,20,0.52)]' : 'rounded-[18px] bg-white p-1'}`}>
                    <img src={logoImage} alt={logoAlt} className="h-full w-full object-contain" />
                  </span>
                  <div>
        ...
        <section className={`${isMechanicPreview ? 'bg-[#f2f0ec]' : 'bg-[#f5f1e8]'} px-5 py-14 sm:px-6 lg:px-8`}>
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <div className={`text-xs font-black uppercase ${isMechanicPreview ? 'text-[#e50914]' : 'text-[#5f7d2f]'}`}>{sectionKicker}</div>
              <h2 className={`mt-5 max-w-2xl text-4xl font-black leading-tight md:text-6xl ${isMechanicPreview ? 'text-[#101113]' : 'text-[#15200f]'}`}>
                {sectionHeadline}
              </h2>
            </div>
              ...
              {preview.variations.map((variation, index) => (
                ...
                >
                  <div className="relative aspect-[16/11] overflow-hidden bg-[#182417]">
                    ...
                    {isMechanicPreview ? (
                      <div className="absolute right-4 top-4 rounded-full border border-white/20 bg-black/48 px-3 py-2 text-[11px] font-black uppercase text-[#ff2a2a] backdrop-blur-md">
                        Option {index + 1}
                      </div>
                    ) : null}
                    <div className="absolute bottom-4 left-4 right-4">
                      <span className={`inline-grid h-10 w-10 place-items-center rounded-full text-sm font-black ${isMechanicPreview ? 'bg-[#e50914] text-white' : 'bg-[#f2c14e] text-[#15200f]'}`}>
                        {index + 1}
                      </span>
                  ...
                  <div className="space-y-5 p-6">
                    ...
                    <div className={`rounded-2xl p-4 ${isMechanicPreview ? 'bg-[#fff0f0]' : 'bg-[#eef4df]'}`}>
                      <div className={`text-xs font-black uppercase ${isMechanicPreview ? 'text-[#e50914]' : 'text-[#5f7d2f]'}`}>Customer action</div>
                      <p className="mt-2 text-sm font-bold leading-6 text-[#172315]">{variation.customerAction}</p>
                    </div>
                    {variation.linkHref ? (
                      <Link
                        href={variation.linkHref}
                        className={`inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full px-5 text-sm font-black transition ${isMechanicPreview ? 'bg-[#e50914] text-white hover:bg-[#ff2a2a]' : 'bg-[#172315] text-white hover:bg-[#2a3a21]'}`}
                      >
                        Open this preview
              ...
              return (
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden bg-[#d9e7c5]">
                    ...
                    <span className={`absolute bottom-4 left-4 grid h-12 w-12 place-items-center rounded-full bg-white shadow-lg ${isMechanicPreview ? 'text-[#e50914]' : 'text-[#386018]'}`}>
        ...
        <section className={`${isMechanicPreview ? 'bg-[#ebe7e1]' : 'bg-[#eef4df]'} px-5 py-16 sm:px-6 lg:px-8`}>
          <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-2">
            <div className="rounded-[28px] bg-white p-6 shadow-[0_18px_60px_rgba(35,48,27,0.1)]">
              <div className={`flex items-center gap-3 text-sm font-black uppercase ${isMechanicPreview ? 'text-[#e50914]' : 'text-[#386018]'}`}>
              ...
              <div className="mt-6 space-y-3">
                {preview.trustPoints.map((point) => (
                  <div key={point} className="flex gap-3 text-sm leading-6 text-[#56614c]">
                    <BadgeCheck className={`mt-0.5 shrink-0 ${isMechanicPreview ? 'text-[#e50914]' : 'text-[#5f8d24]'}`} size={16} aria-hidden="true" />
                    <span>{point}</span>
                  </div>
                ))}
              </div>
